...
#define MAX_THREADS MAX_ORD_PALS + 3
...
pthread_mutex_t mutex_espai = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t mutex_moviment = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t mutex_continuar = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t mutex_teclat = PTHREAD_MUTEX_INITIALIZER;
  ...
  do
  {
    ...
    if ((f_h != ipil_pf) || (c_h != ipil_pc))
    {
      ...
      if (win_quincar(f_h, c_h) == ' ')
      {
        ...
        {
          pthread_mutex_unlock(&mutex_pantalla);
          pthread_mutex_lock(&mutex_continuar);
          cont = ipil_pc;
          pthread_mutex_unlock(&mutex_continuar);
        }
      }
      else
      {
...
void *mou_paleta_usuari(void *cap)
{
  do
  {
    ...
    pthread_mutex_lock(&mutex_teclat);
    tec = win_gettec();
    pthread_mutex_unlock(&mutex_teclat);

    if (tec == TEC_ESPAI)
    {
      pthread_mutex_lock(&mutex_espai);
      espai_premut = (espai_premut + 1) % 2;
      pthread_mutex_unlock(&mutex_espai);
    ...
    if (tec != 0)
    {
      ...
      if ((tec == TEC_AVALL) && (win_quincar(ipu_pf + l_pal, ipu_pc) == ' '))
      {
        ...
        if (moviments > 0)
        {
          pthread_mutex_lock(&mutex_moviment);
          moviments--; 
          pthread_mutex_unlock(&mutex_moviment);
        }
      }
      else
      {
  ...
  do
  {
    ...
    if (f_h != ipo_pf[i])
    {
      ...
      if (v_pal[i] > 0.0)
      {
        ...
        {
          win_escricar(ipo_pf[i], ipo_pc[i], ' ', NO_INV);

          po_pf[i] += v_pal[i];
          ipo_pf[i] = po_pf[i];

          win_escricar(ipo_pf[i] + l_pal - 1, ipo_pc[i], '1' + i, INVERS);
          pthread_mutex_unlock(&mutex_pantalla);
      ...
      else
      {
        ...
        {
          win_escricar(ipo_pf[i] + l_pal - 1, ipo_pc[i], ' ', NO_INV);

          po_pf[i] += v_pal[i];
          ipo_pf[i] = po_pf[i];

          win_escricar(ipo_pf[i], ipo_pc[i], '1' + i, INVERS);
          pthread_mutex_unlock(&mutex_pantalla);
...
int main(int n_args, const char *ll_args[])
{
  ...
  pthread_mutex_init(&mutex_espai, NULL);
  pthread_mutex_init(&mutex_moviment, NULL);
  pthread_mutex_init(&mutex_teclat, NULL);
  pthread_mutex_init(&mutex_continuar, NULL);
  ...
  for (int i = 0; i < n_paletes_ord; i++)
  {
    if (pthread_create(&tid[n_threads], NULL, mou_paleta_ordinador,
                       (void *)(intptr_t)i) == 0)
      n_threads++;
  }
  ...
  pthread_mutex_destroy(&mutex_espai);
  pthread_mutex_destroy(&mutex_moviment);
  pthread_mutex_destroy(&mutex_teclat);
  pthread_mutex_destroy(&mutex_continuar);
